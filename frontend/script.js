...
function loadCategories() {
  ...
  fetch(`/api/categories?type=expense&user_id=${user.id}`)
    ...
    .then(data => {
      ...
      if (!expenseCategorySelect || !budgetCategorySelect) {
  ...
  fetch(`/api/categories?type=income&user_id=${user.id}`)
    ...
    .then(data => {
      ...
      if (!incomeCategorySelect) {
        console.warn('Income category select not found');
        return;
      }
...
function populateCategoryFilter(data) {
  ...
  if (categoryFilter) {  // ADD THIS CHECK
  ...
  if (chartCategoryFilter) {  // ADD THIS CHECK
  ...
  if (barCategoryFilter) {  // ADD THIS CHECK
...
function renderBudgetTracking(budgets) {
  ...
  const filterPeriod = budgetPeriodFilter?.value || 'all';  // ALSO CHANGE THIS LINE
